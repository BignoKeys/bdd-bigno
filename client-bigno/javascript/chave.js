function Identification() {
  ...
  this.internacionalization.base = 'bigno';
  this.species = {};
  this.descriptors = {};
...
Identification.prototype.startup = function() {
  $("#category").html(""); //tag da categoria 
  $("#descritoresSelecionados").html(""); //tag dos descritores selecionados
  $("#especiesElegiveis").html(""); //tag das especies elegíveis
  $("#especiesDescartadas").html(""); //tag das especies descartadas
  this.species = {}; //todas as especies 
  this.descriptors = {}; //todos os descritores
  this.eligibleSpecies = {}; //todas as especies elegíceis
  this.eligibleCategories = {}; //todas as categorias elegíveis
  this.eligibleDescriptors = {}; //todos os descritores elegíveis
  this.eligibleStates = {}; //todos os estados elegíveis
  this.selectedStates = {}; //todos os estados selecionados
  this.definedNumericals = {}; //todos os estados numericos
  ...
  //criação dos descritores
    ...
    //identificar as especies e estados
    self.identify();
  });
}
...
//Modificar para os parametros de bigno
...
Identification.prototype.createSpecies = function(callback){
  var self = this;
  console.log(self);
 $.getJSON("/api/Species?filter[where][base]="+self.base+"&filter[where][language]="+self.internacionalization.language+"&filter[fields]["+self.internacionalization.language+":bigno:Image:vegetativeFeaturesImage]=true&filter[fields]["+self.internacionalization.language+":bigno:Image:flowerImage]=true&filter[fields]["+self.internacionalization.language+":bigno:Image:fruitImage]=true&filter[fields][id]=true&filter[fields]["+self.internacionalization.language+":bigno:Image:distributionImage]=true&filter[fields][id]=true&filter[fields]["+self.internacionalization.language+":bigno:Image:ecologyImage]=true&filter[fields][id]=true&filter[fields]["+self.internacionalization.language+":dwc:Taxon:genus]=true&filter[fields][id]=true&filter[fields]["+self.internacionalization.language+":dwc:Taxon:scientificName]=true&filter[fields]["+self.internacionalization.language+":dwc:Taxon:taxonRank]=true&filter[fields]["+self.internacionalization.language+":dwc:Taxon:scientificNameAuthorship]=true&filter[fields]["+self.internacionalization.language+":bigno:Sample:phenology]=true&filter[order][0]="+self.internacionalization.language+":bigno:Sample:phenology%20ASC&filter[order][1]="+self.internacionalization.language+":dwc:Taxon:scientificName%20ASC"/*, { filter : query }*/, function(species){
        species.forEach(function(sp){
            self.species[sp.id] = {};
            self.species[sp.id].id = sp.id;
            self.species[sp.id].htmlId = sp.id.htmlId();
            ...
            self.species[sp.id].html = $("<div class='especies' id = " + self.species[sp.id].htmlId + "></div>");
            self.species[sp.id].thumbnail = sp[self.internacionalization.language+":bigno:Image:flowerImage"]?sp[self.internacionalization.language+":bigno:Image:flowerImage"].images[0].thumbnail:sp[self.internacionalization.language+":bigno:Image:vegetativeFeaturesImage"]?sp[self.internacionalization.language+":bigno:Image:vegetativeFeaturesImage"].images[0].thumbnail:sp[self.internacionalization.language+":bigno:Image:fruitImage"]?sp[self.internacionalization.language+":bigno:Image:fruitImage"].images[0].thumbnail:sp[self.internacionalization.language+":bigno:Image:ecologyImage"]?sp[self.internacionalization.language+":bigno:Image:ecologyImage"].images[0].thumbnail:sp[self.internacionalization.language+":bigno:Image:distributionImage"]?sp[self.internacionalization.language+":bigno:Image:distributionImage"].images[0].thumbnail:"images/lspm.jpg"
            self.species[sp.id].html.append("<a href='/profile/species/" + self.base + "/" + sp.id + "' target='_blank' ><img id='_img_" + self.species[sp.id].htmlId + "'src='" + self.species[sp.id].thumbnail + "' onerror = 'imageError(this)' /></a>");
            self.species[sp.id].html.append("<div class='nsp'></div>");
...
Identification.prototype.createDescriptors = function(callback) {
  ...
  $.getJSON("/api/Schemas?filter[where][language]="+self.internacionalization.language+"&filter[where][class]=State&filter[order]=order"/*, { filter : query }*/, function(states){
    states.forEach(function(state) {
      var stateImg  = typeof state.images != "undefined" && state.images.length && state.images.length>0 && state.images[0].thumbnail ?state.images[0].thumbnail:"/images/lspm.jpg";
      // CATEGORY
      ...
      if(typeof self.descriptors[state.category][state.field] == "undefined"){
        ...
        self.descriptors[state.category][state.field].html = $("<li><section class='toggle'><span>+</span>" + self.descriptors[state.category][state.field].value + "<a target='_blank' href='/profile/glossary/"+self.base+"/"+self.descriptors[state.category][state.field].value+"'><img alt='"+self.descriptors[state.category][state.field].value+"|"+self.descriptors[state.category].value+"' src='/images/"+self.base+"_glo.png' class='lala'></a></section><div id='"+self.descriptors[state.category][state.field].htmlId+"'class='valoresi inner show' style='display: block;'></div></li>");
...
Identification.prototype.printDescriptors = function() {
  var self = this;
  console.log("Printar categorias");
  ...
  Object.keys(self.descriptors).forEach(function(idCategory) {
    ...
    var descriptorCount = 0; //contador de descritores
    Object.keys(self.descriptors[idCategory]).forEach(function(idDescriptor) {
      ...
      var stateCount = 0; //contador de estados 
      Object.keys(self.descriptors[idCategory][idDescriptor]).forEach(function(idState) {
        if(typeof self.descriptors[idCategory][idDescriptor][idState].value != "undefined"){
          if(self.descriptors[idCategory][idDescriptor][idState].value){
            ...
            if(self.eligibleStates[self.descriptors[idCategory][idDescriptor][idState].id] && (self.eligibleStates[self.descriptors[idCategory][idDescriptor][idState].id].count==null || self.eligibleStates[self.descriptors[idCategory][idDescriptor][idState].id].count<=Object.keys(self.eligibleSpecies).length)){
              ...
              //Deleta descritor com nenhum estado
            ...
            if(self.selectedStates[self.descriptors[idCategory][idDescriptor][idState].id]){
              ...
              $("#count_"+self.descriptors[idCategory][idDescriptor][idState].htmlId).html("<br><a href='#' onclick='identification.unselectState(\""+self.descriptors[idCategory][idDescriptor][idState].id+"\")'><img style='width:20px;margin-top:5px' src='/images/"+self.base+"_x.png'/></a>")
            }
          }
        }
      });
